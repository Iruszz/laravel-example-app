...
const fetchCategories = async () => {
    try {
        ...
        if (error.response && error.response.status === 401) {
            ...
            setMessage('Er is iets misgegaan bij het ophalen van de categories.');
        }
    }
};
...
const deleteCategory = (id: number) => {
    categoryStore.actions.delete(id);
};
...
<template>
    ...
    <main class="flex-1 flex flex-col overflow-hidden">
        <div v-if="categories.length > 0" class="flex-1 flex flex-col overflow-hidden mt-10 shadow-md sm:rounded-lg">
            <div class="flex-1 overflow-x-auto">
                ...
                <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                    ...
                    <tbody>
                        <tr v-for="category in categories" :key="category.id" class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 border-gray-200 hover:bg-gray-50 dark:hover:bg-gray-600">
                            <td class="w-4 p-4">
                                <div class="flex items-center">
                            ...
                            <!-- <th class="flex items-center px-6 py-4 gap-3">
                                ...
                                <div class="text-sm font-medium text-gray-800 dark:text-white">{{ category.user.name }}</div>
                            </th> -->
                            <td class="px-6 py-4">
                                <RouterLink
                            ...
                            <!-- <td class="px-6 py-4">
                                <div class="flex items-center">
                                    <img class="w-10 h-10 rounded-full object-cover" src="https://flowbite.com/docs/images/examples/image-2@2x.jpg" alt="Jese image">
                                    <div class="ps-3">
                                        <div :class="category.agent ? 'dark-text-gray-800 dark:text-white' : 'text-gray-500 dark:text-red'">
                                            {{ category.agent ? category.agent.name : 'No Agent' }}
                                        </div>
                                    </div>  
                                </div>
                            </td> -->
                            ...
                            <td class="px-6 py-4">
                                <button
                                    :id="`dropdownDefault-${category.id}`"
                                ...
                                >
                                    <ul class="py-2 text-sm font-medium text-gray-700 dark:text-gray-400" aria-labelledby="dropdownMenuIconHorizontalButton">
                                        <!-- <li>
                                            ...
                                            @click="markAsSolved( category.id )">
                                        ...
                                        <li>
                                            <RouterLink v-if="currentUser?.is_admin" class="flex block px-4 py-2 mx-2 gap-2 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white"
                                                :to="{ name: 'category.assign', params: { id: category.id } }">
