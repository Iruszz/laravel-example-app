...
import { setMessage, destroyMessage, setErrorBag } from '../../../services/error';
...
const users = computed(() =>
    orderBy(userStore.getters.all.value, 'name')
);
...
const fetchUsers = async () => {
    try {
        ...
        if (error.response && error.response.status === 401) {
            ...
            setMessage('Er is iets misgegaan bij het ophalen van de users.');
        }
    }
};
...
const deleteUser = async (id: number) => {
    try {
        ...
        setMessage(error.response?.data.message || 'User deletion failed.');
    }
};

function deleteConfirm(userId: number) {
    if (confirm("The user is being deleted together with the reviews")) {
        deleteUser(userId);
    }
}
</script>
...
<template>
    ...
    <main class="flex-1 flex flex-col overflow-hidden">
        <div
            v-if="users.length > 0"
            class="flex-1 flex flex-col overflow-hidden mt-10 shadow-md sm:rounded-lg"
        >
            <div class="flex-1 overflow-x-auto">
                <div
                    class="flex h-full items-center justify-between flex-column flex-wrap md:flex-row space-y-4 md:space-y-0 px-15 pb-4 bg-white dark:bg-gray-900"
                >
                    <div>
                        <button
                    ...
                    <tbody>
                        ...
                        >
                            ...
                            <th class="flex items-center px-6 py-4 gap-3">
                                <img class="w-10 h-10 rounded-full object-cover" src="https://flowbite.com/docs/images/examples/image-2@2x.jpg" alt="User image"/>
                            ...
                            <td class="px-6 py-4">
                                ...
                                >
                                    <ul class="py-2 text-sm font-medium text-gray-700 dark:text-gray-400" aria-labelledby="dropdownMenuIconHorizontalButton">
                                        ...
                                        <li v-if="getLoggedInUser?.is_admin">
                                            <button
                                                type="button"
                                                class="flex block px-4 py-2 mx-2 gap-2 rounded-lg text-sm text-red-700 hover:bg-red-100 dark:hover:bg-gray-600 dark:text-red-500"
